cometPlot2 <- function(ref, alt, ploid=2, gfreq=NULL, file=NULL, maxdepth=500, maxSNPs=1e5, ...){
  ...
  maxdepth <- max(ref,alt)
  ran <- max(abs(newMat),na.rm=T)
  
  par(mar = c(5.1,5.1,1.1,1.1)*sqrt(cex), ...)
  image(0:maxdepth,0:maxdepth,t(newMat[0:maxdepth+1,0:maxdepth+1]),  zlim=c(-ran,ran), col=newCol,
        xaxt = "n", yaxt="n", ylab="Reference allele count", xlab="Alternate allele count")
  ticks <- seq(par()$xaxp[1],par()$xaxp[2],par()$xaxp[2]/par()$xaxp[3])
  axis(side = 1, at = ticks+0,labels = ticks, cex.axis=cex)
  axis(side = 2, at = ticks+0,labels = ticks, cex.axis=cex)
  grid()
  
  legend_image <- as.raster(matrix(rev(newCol), ncol = 1))
  adj = min(max(ref,alt),maxCount)*0.05
  rasterImage(legend_image,xleft = min(max(ref,alt),maxCount)-adj*2, ybottom = adj, ytop = 7*adj, xright = min(max(ref,alt),maxCount)-adj )
  num <- unique(c(-(exp(seq(max(abs(newMat),na.rm=T),0, length.out=4))-1),exp(seq(0,max(abs(newMat), na.rm=T),length.out=4))-1))
  text(x = min(max(ref,alt),maxCount)-adj*2, y = adj + seq(0,6*adj,length.out=7), labels = format(num,digits=1, scientific=F),cex=cex, pos=2)
  abline(0,1)
  
  matrix(0:maxdepth, nrow=maxdepth+1, ncol=maxdepth+1)
  
  rasterImage(legend_image,xleft = min(max(ref,alt),maxCount)-adj*2, ybottom = adj, ytop = 5*adj, xright = min(max(ref,alt),maxCount)-adj )
  text(x = min(max(ref,alt),maxCount)-adj*2, y = adj + seq(0,4*adj,length.out=6), labels = format(exp(seq(1,maxDiff,length.out=6)),digits=1, scientific=F),cex=cex, pos=2)
  if(scaled){
    newMat <- (obsTab-tempMat)
    ...
    newMat2[indx] <- -(abs(newMat2[indx]))^(1/3)
    indx <- which(newMat2>=0)
    newMat2[indx] <- (newMat2[indx])^(1/3)    
    ran <- max(abs(newMat2),na.rm=T)
    image(0:maxdepth,0:maxdepth,t(newMat2),  zlim=c(-ran,ran), col=newCol,
          xaxt = "n", yaxt="n", ylab="Reference allele count", xlab="Alternate allele count")
    ticks <- seq(par()$xaxp[1],par()$xaxp[2],par()$xaxp[2]/par()$xaxp[3])
    axis(side = 1, at = ticks+0,labels = ticks, cex.axis=cex)
    axis(side = 2, at = ticks+0,labels = ticks, cex.axis=cex)
    ...
    adj = min(max(ref,alt),maxCount)*0.05
    rasterImage(legend_image,xleft = min(max(ref,alt),maxCount)-adj*2, ybottom = adj, ytop = 7*adj, xright = min(max(ref,alt),maxCount)-adj )
    num <- unique(c(-seq(1,0, length.out=4)^(3),seq(0,1,length.out=4)^3))
    text(x = min(max(ref,alt),maxCount)-adj*2, y = adj + seq(0,6*adj,length.out=7), labels = format(num,digits=1, scientific=F),cex=cex, pos=2)
